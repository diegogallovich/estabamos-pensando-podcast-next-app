    ...
    <svg aria-hidden="true" {...props}>
      <defs>
        ...
        <linearGradient id={`${id}-gradient`}>
          <stop offset="0%" stopColor="#D94646" />
          <stop offset="50%" stopColor="#E85C5C" />
          <stop offset="100%" stopColor="#FF7171" />
        </linearGradient>
        <mask id={`${id}-mask`}>
  ...
  return (
    <section {...props}>
      <h2 className="flex items-center font-mono text-sm font-medium leading-7 text-slate-900">
        <TinyWaveFormIcon
          colors={['fill-red-500', 'fill-pink-300']}
          className="h-2.5 w-2.5"
        />
        <span className="ml-2.5">Sobre el podcast</span>
      </h2>
      <p
        className={clsx(
      ...
      >
        {`"Estábamos Pensando" es un podcast donde dos amigos despliegan charlas
        estimulantes sobre digitalización de negocios, tecnología, y
        ocasionalmente se adentran en reflexiones filosóficas y sociales. Una
        ventana a conversaciones genuinas que merecen ser compartidas.`}
      </p>
      {!isExpanded && (
        <button
          type="button"
...
export function Layout({ children }) {
  let hosts = ['Leonardo Bordones', 'Diego Gallovich']
        ...
        <div className="relative z-10 mx-auto px-4 pb-4 pt-10 sm:px-6 md:max-w-2xl md:px-4 lg:min-h-full lg:flex-auto lg:border-x lg:border-slate-200 lg:px-8 lg:py-12 xl:px-12">
          ...
          <div className="mt-10 text-center lg:mt-12 lg:text-left">
            <p className="text-xl font-bold text-slate-900">
              <Link href="/">Estabamos Pensando</Link>
            </p>
            <p className="mt-3 text-lg font-medium leading-8 text-slate-700">
              Conversaciones digitales entre amigos, para amigos.
              Digitalizacion, negocios, tecnologia, y la ocasional reflexion
              filosofica y social.
            </p>
          </div>
